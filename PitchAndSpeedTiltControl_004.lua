...
function update()
  ...
  local mul_fact = 0.5 -- multiplying factor (q) 0.1 to 2 range
   ...
   if (pit_ang >= -lmt_ang and pit_ang <= lmt_ang) then -- Min And Max angle limit Condition  
       -- if arming:is_armed() then 
            ...
            if(drone_speed) then
                ...
                gcs:send_text(3,"drone_speed = "..drone_speed)
                gcs:send_text(3,"pit_spd_pwm = "..pit_spd_pwm)
                gcs:send_text(3,"pit_ang = "..pit_ang*(180/pi))
                gcs:send_text(3,"spd_ang = "..spd_ang*(180/pi))
                gcs:send_text(3,"pit_spd_ang = "..pit_spd_ang*(180/pi))

               if(pit_spd_ang >=  -lmt_ang and pit_spd_ang <=  lmt_ang) then
